...
async def global_nearby_search(
    lat: float,
    lng: float,
    ...
    """
    Search global geo-index for places within radius of given coordinates.

    Uses GEORADIUS for O(log N + M) radius queries.
    ...
    try:
        ...
        # Extract place_ids and distances
        place_distances = {}
        place_ids = []
        for item in geo_results:
            # item is (place_id, distance) when withdist=True
            if isinstance(item, (list, tuple)) and len(item) >= 2:
                pid, dist = item[0], float(item[1])
            else:
                # Fallback for different redis library versions
                pid, dist = item, None
            place_ids.append(pid)
            place_distances[pid] = int(dist) if dist is not None else None
        ...
        # Build results (already sorted by distance from GEORADIUS)
        results = []
        for i, meta in enumerate(metadata_results):
            if not meta:
                continue
            ...
            place_id = place_ids[i]
            place_lat = float(meta.get("lat", 0))
            place_lng = float(meta.get("lng", 0))
            ...
            results.append({
                ...
                "latitude": place_lat,
                "longitude": place_lng,
                "distance_meters": place_distances.get(place_id),
                "bounce_count": int(meta.get("bounce_count", 0)),
                "photo_url": meta.get("photo_url"),
                "types": types
            })
        ...
        logger.error(f"Global nearby search failed: {e}")
        return [], False
